...
void ScaleModule::process(Image &img)
{
	...
	double redWB, greenWB, blueWB;
	calcWhiteBalance(redWB, greenWB, blueWB);
	RawDev::verbout << "White balance scales are";
	RawDev::verbout.newline();
	printScale("R =", redWB); // Scales on newline
	printScale("G =", greenWB);
	printScale("B =", blueWB, true);

	// Compute scales
	double scaleR, scaleG, scaleB; // Precomputed scales
	const Color::RGB64 white = m_CamProfile->getWhiteLevel();
	scaleR = redWB * baseExposure / (white.r - black);
	scaleG = greenWB * baseExposure / (white.g - black);
	scaleB = blueWB * baseExposure / (white.b - black);
...
Color::RGB64 ScaleModule::estimateBlackPoint(const Image &img)
{
	...
	if (active.top >= 3 * pad && active.left >= 3 * pad)
	{
		Rect masked[2] = {     // Masked areas calc from active
			Rect(pad, pad, active.right - pad, active.top - pad),
			Rect(pad, active.top - pad, active.left - pad, active.bottom - pad)
		};
...
/// </summary>
/// <param name="img">Raw image</param>
/// <param name="masked">Masked pixel rectangles</param>
...
/// </summary>
/// <param name="redWB">Red scaling output</param>
/// <param name="greenWB">Green scaling output</param>
/// <param name="blueWB">Blue scaling output</param>
void ScaleModule::calcWhiteBalance(
	double &redWB, double &greenWB, double &blueWB)
{
	// Compute white balance
	Color::RGB64 referenceNeutral = (m_CamProfile->getAnalogBalanceMatrix()
		* m_CamProfile->getCamCalibrationMatrix()).inverse()
		* calcCameraNeutral();
	applyTintCorrection(referenceNeutral); // Correct Green/Magenta cast

	// Invert, normalize and save
	const double maxRef = Utils::max3( // Max reference channel
		referenceNeutral.r, referenceNeutral.g, referenceNeutral.b);
	redWB = maxRef / referenceNeutral.r;
	greenWB = maxRef / referenceNeutral.g;
	blueWB = maxRef / referenceNeutral.b;
	return;
}
...
/// </summary>
/// <returns>Camera neutral</returns>
Color::RGB64 ScaleModule::calcCameraNeutral()
{
	// Calculate CCT to XYZ
	Color::CIEXYZ camNeutralXYZ = Color::cct2XYZ(m_ColorTemp);

	// Create color matrix
	const Mat3x3 color = m_CamProfile->getColorMatrix();
	const Mat3x3 XYZtoCamera = m_CamProfile->getAnalogBalanceMatrix()
		* m_CamProfile->getCamCalibrationMatrix() * color;
	return XYZtoCamera * camNeutralXYZ;
}

/// <summary>
/// Tint correction to reduce green/magenta casts
/// </summary>
/// <param name="refNeutral">Reference camera neutral</param>
/// <remarks>
/// Only simple solution for now.
/// The best were to compute the isotherm.But that's not easy!
/// Simple but smart way... Hint: green clip first!
/// </remarks>
void ScaleModule::applyTintCorrection(Color::RGB64 &refNeutral)
{
	const double mult = 1.0 + (fabs(m_Tint) / 100.0);

	// Adding green (increase R, B)
	if (m_Tint > 0) {
		refNeutral.r /= mult;
		refNeutral.b /= mult;
	}
	// Adding magenta (only decrease G)
	else if (m_Tint < 0) {
		refNeutral.g /= mult;
	}
	return;
