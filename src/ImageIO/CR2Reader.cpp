...
/// </summary>
/// <remarks>
/// Needed for bit depth, components and sampling freq.
/// </remarks>
...
void CR2Reader::readTiffHeader(TiffHeader& th)
{
	...
	if (dataOK == false)
	{
		throw FormatException(CR2Reader::kModuleName,
			m_fileName, "Wrong or unsupported data in the TIFF header.");
	}
...
void CR2Reader::read(Array2D<uint16_t>& img)
{
	...
	modelCorrect(width, height, slices); // Model correcttion
	img(width, height); // Allocate new image
...
void CR2Reader::readDHTheader(DHTHeader& hdr)
{
	...
	// Read len of DHT
	uint16_t len; // Length of the header
	m_file.read(reinterpret_cast<char*>(&len), sizeof(len));
	if (m_file.good() == false)
	{
		throw IOException(CR2Reader::kModuleName,
			m_fileName, "Failed to read DHT header length.");
	}
	...
	{	// Minimal size of DHT
		throw FormatException(CR2Reader::kModuleName,
			m_fileName, "DHT header length too small.");
	}
	...
	{
		throw IOException(CR2Reader::kModuleName,
			m_fileName, "Decoded DHT length doesn't match the given length.");
	}
	return;
}
...
int CR2Reader::makeHuffmanTree(DHTHeader& hdr, char* base)
{
	...
	{
		// All tables must be DC=0 for lossless
		throw FormatException(CR2Reader::kModuleName,
...
void CR2Reader::readSOF3header(SOF3Header& hdr)
{
	...
	{
		throw FormatException(CR2Reader::kModuleName,
			m_fileName, "Wrong SOF3 header length value.");
	}
...
void CR2Reader::readSOSheader(SOSHeader& hdr, const SOF3Header& sof3)
{
	...
	{
		throw FormatException(CR2Reader::kModuleName,
			m_fileName, "Wrong SOS header length.");
	}
	...
	{
		throw FormatException(CR2Reader::kModuleName,
			m_fileName, "Last SOS constant data doesn't match.");
	}
	return;
}
...
/// work, but I haven't tested it yet. Maybe in the future!
/// </remarks>
