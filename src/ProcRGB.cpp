...
void ProcRGBModule::processImage(Image &img, bool process)
{
	...
	const Mat3x3 cam2work = Color::XYZtoProPhotoRGB
		* profile->getForwardMatrix() * (profile->getAnalogBalanceMatrix()
		* profile->getCamCalibrationMatrix()).inverse();
	...
	for (int row = 0; row < height; row++) {
		for (int col = 0; col < width; col++) {
			v = img.getValue(row, col);

			// Convert from camera to working color space
			v = cam2work * v;
			v.r = Utils::clip(v.r, 0.0, 1.0);
			v.g = Utils::clip(v.g, 0.0, 1.0);
			v.b = Utils::clip(v.b, 0.0, 1.0);
...
/// <param name="outBlack">Output black</param>
...
/*
Basic tone curve borrowed from RAWTherapee (GPLv3)
TODO: Create my own curve generator that is better
*/
