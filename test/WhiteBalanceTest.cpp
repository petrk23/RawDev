...
// clang-format off
// NOLINTBEGIN(modernize-use-designated-initializers, modernize-use-std-numbers)
...
// NOLINTEND(modernize-use-designated-initializers, modernize-use-std-numbers)
// clang-format on
...
{
    constexpr auto sample = k_td[0];

    const WhiteBalance wb(sample.temp, sample.tint);
    EXPECT_DOUBLE_EQ(wb.getColorTemperature(), sample.temp);
    EXPECT_EQ(wb.getTint(), sample.tint);

    EXPECT_THROW(WhiteBalance(std::nan(""), sample.tint), std::invalid_argument);
    EXPECT_THROW(WhiteBalance(1500, sample.tint), std::invalid_argument);
    EXPECT_THROW(WhiteBalance(100'000, sample.tint), std::invalid_argument);
    EXPECT_THROW(WhiteBalance(sample.temp, -200), std::invalid_argument);
    EXPECT_THROW(WhiteBalance(sample.temp, 200), std::invalid_argument);
}
...
TEST(WhiteBalanceTest, ScaleTest)
{
    for (const auto& ti : k_td) {
        const auto camProfile =
            CamProfile::MakeCamProfile(ti.camID, ti.temp);
        EXPECT_EQ(ti.camID, camProfile->getCameraID());

        // Calculate the target scales
        const WhiteBalance wb(ti.temp, ti.tint);
        const WhiteBalance::Scale sc = wb.calcScales(*camProfile);

        // Compare with the ref
        constexpr double tolerance = 1e-12;
        EXPECT_NEAR(ti.sc.rs, sc.rs, tolerance);
        EXPECT_NEAR(ti.sc.gs, sc.gs, tolerance);
        EXPECT_NEAR(ti.sc.bs, sc.bs, tolerance);
        EXPECT_GE(sc.rs, 0.0);
        EXPECT_GE(sc.gs, 0.0);
        EXPECT_GE(sc.bs, 0.0);
    }
